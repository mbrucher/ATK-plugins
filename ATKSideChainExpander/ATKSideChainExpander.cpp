  ...
  volumesplitFilter(4), applyGainFilter(2), volumemergeFilter(2), drywetFilter(2), outLFilter(nullptr, 1, 0, false), outRFilter(nullptr, 1, 0, false), sidechain(false)
{
  ...
  gainColoredExpanderFilter1.set_input_port(0, &powerFilter1, 0);
  attackReleaseFilter1.set_input_port(0, &gainColoredExpanderFilter1, 0);
  ...
  gainColoredExpanderFilter2.set_input_port(0, &powerFilter2, 0);
  attackReleaseFilter2.set_input_port(0, &gainColoredExpanderFilter2, 0);
...
void ATKSideChainExpander::Reset()
{
  ...
  if (sampling_rate != endpoint.get_input_sampling_rate())
  {
    ...
    gainColoredExpanderFilter1.set_input_sampling_rate(sampling_rate);
    gainColoredExpanderFilter1.set_output_sampling_rate(sampling_rate);
    ...
    gainColoredExpanderFilter2.set_input_sampling_rate(sampling_rate);
    gainColoredExpanderFilter2.set_output_sampling_rate(sampling_rate);
...
void ATKSideChainExpander::OnParamChange(int paramIdx)
{
  ...
  case kLinkChannels:
    ...
    {
      gainColoredExpanderFilter1.set_input_port(0, &sumFilter, 0);
  ...
  case kThreshold1:
    gainColoredExpanderFilter1.set_threshold(std::pow(10, GetParam(kThreshold1)->Value() / 10));
    break;
  case kRatio1:
    gainColoredExpanderFilter1.set_ratio(GetParam(kRatio1)->Value());
    break;
  case kSoftness1:
    gainColoredExpanderFilter1.set_softness(std::pow(10, GetParam(kSoftness1)->Value()));
    break;
  case kColor1:
    gainColoredExpanderFilter1.set_color(GetParam(kColor1)->Value());
    break;
  case kQuality1:
    gainColoredExpanderFilter1.set_quality(GetParam(kQuality1)->Value());
    break;
  case kPower1:
  {
    auto power = GetParam(kPower1)->Value();
  ...
  case kMakeup1:
    ...
    break;
  case kThreshold2:
    gainColoredExpanderFilter2.set_threshold(std::pow(10, GetParam(kThreshold2)->Value() / 10));
    break;
  case kRatio2:
    gainColoredExpanderFilter2.set_ratio(GetParam(kRatio2)->Value());
    break;
  case kSoftness2:
    gainColoredExpanderFilter2.set_softness(std::pow(10, GetParam(kSoftness2)->Value()));
    break;
  case kColor2:
    gainColoredExpanderFilter2.set_color(GetParam(kColor2)->Value());
    break;
  case kQuality2:
    gainColoredExpanderFilter2.set_quality(GetParam(kQuality2)->Value());
    break;
  case kPower2:
  {
