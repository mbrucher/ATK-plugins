...
ATKUniversalDelay::ATKUniversalDelay(IPlugInstanceInfo instanceInfo)
  ...
  inFilter(NULL, 1, 0, false), outFilter(NULL, 1, 0, false), delayFilter(1024)
{
  TRACE;

  //arguments are: name, defaultVal, minVal, maxVal, step, label
  GetParam(kDelay)->InitDouble("Delay", 1, 0.001, 10.0, 0.001, "ms");
  GetParam(kDelay)->SetShape(2.);
  GetParam(kBlend)->InitDouble("Blend", 100, 0., 100.0, 0.01, "%");
  GetParam(kBlend)->SetShape(1.);
  GetParam(kFeedforward)->InitDouble("Feedforward", 50., -99.9, 99.9, 0.01, "%");
  GetParam(kFeedforward)->SetShape(1.);
  GetParam(kFeedback)->InitDouble("Feedbak", 0., -99.9, 99.9, 0.01, "%");
